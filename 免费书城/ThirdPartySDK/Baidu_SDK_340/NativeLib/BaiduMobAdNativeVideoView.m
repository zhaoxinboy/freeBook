...
@property (nonatomic, strong)   UISlider *progress;//播放进度
@property (nonatomic, strong)   UIButton *btnPlayOrPause; //播放/暂停按钮
@property (nonatomic, strong)   UIButton *btnFullScreen; //fullscreen按钮
@property (nonatomic, strong)   UIButton *btnDone; //全屏时back按钮
@property (nonatomic, strong)   UIView *controlView;//包含fullscreen按钮，播放按钮，进度按钮
...
@property (nonatomic, strong)   BaiduMobAdNativeAdObject *associatedObject;
@property (nonatomic, strong) UIView            *displayView;//视频展现的view
@property (nonatomic, strong) NSURL             *url;//视频url
@property (nonatomic, strong) AVPlayer          *player;
@property (nonatomic, strong) AVPlayerLayer     *playerLayer;
@property (nonatomic, strong) UITapGestureRecognizer *tapGesture;//点击视频区域隐藏和显示控制条
@property (nonatomic, weak) UIView *originSuperView;//非全屏superview
...
- (void)play {
    [self sendVideEvent:onClickToPlay currentTime:0];
    self.hidden = NO;
    if (_state == INIT) {
        [self startPlayback];
...
- (void)stop {
    if (self.player && _state != COMPLETE) {
        if (_state != INIT) {
            [self sendVideEvent:onClose currentTime:[self currentTime]];
        }
...
- (void)onPlayerItemDidReachEnd:(NSNotification *)notification {
    ...
    [self sendVideEvent:onComplete currentTime:[self currentTime]];
}
...
- (void)onPlayerRateChange {
    if (_state == INIT && self.player.rate > 0) {
        _state = PLAYING;
        _btnLP.hidden = NO;
        [self sendVideEvent:onStart currentTime:0];
    }
...
- (void)playbackFail:(NSString *)errorData {
    [self completePlayback];
    [self sendVideEvent:onError currentTime:[self currentTime]];
}
...
//    [[UIApplication sharedApplication] setStatusBarHidden:!self.isFullScreen withAnimation:UIStatusBarAnimationNone];
    ...
    if (self.isFullScreen) {
        ...
        [self sendVideEvent:onFullScreen currentTime:[self currentTime]];
...
- (void)videoAdClick {
    [self sendVideEvent:onClick currentTime:[self currentTime]];
}
...
#pragma mark - sendVideEvent
- (void)sendVideEvent:(BaiduAdNativeVideoEvent)event currentTime:(NSTimeInterval) currentTime {
    if (event == onClick) {
